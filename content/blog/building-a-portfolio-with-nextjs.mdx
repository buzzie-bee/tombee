...
Even with me in the 'driver's seat' the current models _still_ need a lot of handholding and guidance. Luckily the tooling for doing this is getting better and it's now possible to structure a codebase and enforce rules so that the models produce "not-bad" results first time.
